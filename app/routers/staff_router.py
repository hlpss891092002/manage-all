...
async def get_latest(payload  : Annotated[dict, Depends(user_validation)]):
   try:
      ...
      def run_threads():
         start = time()
         exec_yesterday_produce_def = threading.Thread(target=get_yesterday_produce_category_sort)
         exec_stock_category_def = threading.Thread(target= get_stock_category_sort)
         exec_ready_stock_def = threading.Thread(target= get_ready_stock_sort)
         exec_week_output = threading.Thread(target=get_seven_days_outs_sort)
         exec_yesterday_produce_def.start()
         exec_stock_category_def.start()
         exec_ready_stock_def.start()
         exec_week_output.start()
         exec_yesterday_produce_def.join()
         exec_stock_category_def.join()
         exec_ready_stock_def.join()
         exec_week_output.join()
         end = time()
